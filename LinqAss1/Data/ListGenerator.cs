...
namespace C_Assignment.Data
{
	internal class ListGenerator
	{
		...
		static ListGenerator()
		{
			...
			//#region Query Syntax Read
			//CustomerList = (from C in XDocument.Load("Customers.xml")
			//			 .Root?.Elements("customer")
			//				select new Customer()
			//				{
			//					CustomerID = (string?)C.Element("id") ?? "UnKnown",
			//					CustomerName = (string?)C.Element("name") ?? "No Name",
			...
			//								  OrderDate = DateTime.TryParse((string?)O.Element("orderdate"), out DateTime Dt) ? Dt : DateTime.MinValue,
